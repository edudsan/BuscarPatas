      ...
      if (result.isConfirmed) {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(errorData.error || 'Não foi possível registrar a adoção.');
          }
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg" centered>
      <Modal.Header closeButton>
        <Modal.Title className="fw-bold">{capitalizeFirstLetter(pet.nome)}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Row>
          <Col md={6}>
            {imagens.length > 0 ? (
              <Carousel>{imagens.map((url, index) => (<Carousel.Item key={index}><img className="d-block w-100 rounded" src={url} alt={`Foto ${index + 1} de ${pet.nome}`} style={{ height: '400px', objectFit: 'cover' }} /></Carousel.Item>))}</Carousel>
            ) : (
              <img className="d-block w-100 rounded" src="https://via.placeholder.com/400x400?text=Sem+Foto" alt="Sem foto" style={{ height: '400px', objectFit: 'cover' }} />
            )}
          </Col>
          <Col md={6}>
      ...
      <Modal.Footer>
        ...
        <Button variant="primary" className="btn-principal" onClick={handleAdotarClick}>Quero Adotar!</Button>
      </Modal.Footer>
    </Modal>
  );
}