...
export function Navbar() {
  ...
  return (
    <BSNavbar collapseOnSelect expand="lg" bg="dark" variant="dark" className="px-3">
        ...
        <Nav className="ms-auto d-flex align-items-center justify-content-center justify-content-lg-end">
          {token ? (
            <Dropdown align="end" className="user-dropdown">
              <Dropdown.Toggle as={Button} variant="link" id="dropdown-basic" className="text-light nav-link-custom d-flex align-items-center">
                <FontAwesomeIcon icon={faUserCircle} className="me-2" size="lg" />
                <span className="d-none d-lg-inline">{userName.split(' ')[0]}</span> {/* Exibe só o primeiro nome em telas maiores */}
              </Dropdown.Toggle>
            ...
            <>
              <Button as={Link} to="/cadastro" variant="outline-light" className="me-lg-3 btn-custom">
                Cadastre-se
              </Button>
              <Button as={Link} to="/login" variant="light" className="btn-custom mt-2 mt-lg-0">
                Entrar
              </Button>
            </>
          )}
