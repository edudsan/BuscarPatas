      ...
      <Card.Body>
        <Stack gap={2} className="text-center align-items-center">
          ...
          <div className="user-details-card">
            <h5 className="user-name-card mb-0">{capitalizeFirstLetter(user.nome)}</h5>
            <p className="user-email-card text-muted mb-0">{user.email}</p>
            <p className="user-phone-card text-muted">{formatPhoneNumber(user.telefone)}</p>
          </div>
        </Stack>
        ...
        <div className="user-actions-card">
          ...
          <Stack direction="horizontal" gap={2} className="justify-content-center">
            <Button variant="link" className="action-btn-card" onClick={() => onEdit(user)}>
              <FontAwesomeIcon icon={faPenToSquare} />
            </Button>
            <Button variant="link" className="action-btn-card text-danger" onClick={() => onDelete(user.adotante_id, user.nome)}>
